...
# Fonction pour se déplacer en Y (HAUT/BAS), avec step et dir
def moveY(step, direction):	# moveY(400, "cw") or "ccw"
	for i in range(step):
		...
		sleep(vitesse)		
	# sleep(1) Commenté pour le test, laisser en vrai ?
      
# Fonction pour se déplacer en X (DROITE/GAUCHE), avec step et dir
def moveX(step, direction):	# moveZ(400, "cw") or "ccw"
	for i in range(step):
		if(direction == "cw"):
